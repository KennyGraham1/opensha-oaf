...
/**
 * This class is a stochastic ETAS catalog that represents an extension of the
 * observed catalog,
 * supplied as mainshock and aftershocks objects. Time is relative to the
 * mainshock. If nSims is supplied,
 * a suite of catalogs will be built. Only the last catalog to be built is
...
public class ETAScatalog {
	...
	public ETAScatalog(double[] ams_vec, double[] a_vec, double[] p_vec, double[] c_vec, double[][][][] likelihood,
			...
			double dataStart, double dataEnd, double forecastStart, double forecastEnd, double Mc, double minMagLimit,
			double maxMagLimit, int maxGenerations, int nSims, boolean validate) {
		...
		if (D)
			System.out.println("ETAS simulation params: alpha=" + alpha + " b=" + b + " Mref=" + refMag + " Mc=" + Mc
					+ " minSimMag=" + minMagLimit + " Mmax=" + maxMagLimit + " nSims=" + nSims);

		List<float[]> newEqList = new ArrayList<float[]>(); // catalog containing list of {time, mag, gen}
		// List<float[]> newEqTimesList = new ArrayList<float[]>(); //catalog containing
		// only {time}

		List<List<float[]>> catalogList = new ArrayList<List<float[]>>(); // list of catalogs
		// List<List<float[]>> catalogTimesList = new ArrayList<List<float[]>>(); //
		// just the times
		...
		if (D)
			System.out.println(
					"Calculating " + nSims + " " + (int) (forecastEnd - forecastStart) + "-day ETAS catalogs...");
		...
		if (nSims > 0) {
			...
			long toc;
			double timeEstimate;
			String initialMessageString = "Calculating " + nSims + " " + (int) (forecastEnd - forecastStart)
					+ "-day ETAS catalogs. ";
			...
			for (int i = 0; i < nSims; i++) {
				...
				if (D && Math.floorMod(i, nSims / 10) == 0)
					System.out.println("Parameter set " + i + ": " + ams_sample + " " + a_sample + " " + p_sample + " "
							+ c_sample);
				// if (D) System.out.println("Parameter set " + i + ": " + ams_sample + " " +
				// a_sample + " " + p_sample + " " + c_sample);

				// Currently sets the first event as mainshock and adjusts magnitude
				// todo step1: change magnitude of LARGEST earthquake
				// todo step2: depending on the total number of events, adjust N-largest
				// magnitudes
				...
				// newEqTimesList = getNewETAScatalogTimes(simulationMainshock, aftershocks,
				// a_sample, p_sample, c_sample, i);
				...
				// instead of adding the new catalog to the list, just compute a histogram of
				// event times. (Magnitude isn't even required)

			}
			toc = watch.elapsed(TimeUnit.SECONDS);
			if (D)
				System.out.println("It took " + toc + " seconds to generate stochastic catalogs.");
			watch.stop();
	...
	// public List<float[]> getNewETAScatalogTimes(ObsEqkRupture mainshock,
	// ObsEqkRupList aftershocks, double a_sample, double p_sample, double c_sample,
	// int simNumber){
	//
	// //extract magnitudes and times from supplied Eqk rupture objects to make
	// catalog (combine MS and AS's)
	...
	//
	// //double[] event = {0, mainshock.getMag(), 0}; //utility variable : {relative
	// time, magnitude, generation number}
	//
	...
	// event[0] = (float) ((rup.getOriginTime() -
	// t0)/ETAS_StatsCalc.MILLISEC_PER_DAY); //elapsed time in days
	...
	//
	// //add children
	// newEqList = getChildren(newEqList, event[0], event[1], (int)event[2],
	// a_sample, p_sample, c_sample, simNumber);
	//
	// }else{
	...
	public List<float[]> getNewETAScatalog(ObsEqkRupture mainshock, ObsEqkRupList aftershocks, double a_sample,
			double p_sample, double c_sample, int simNumber) {
	...
	private List<float[]> getChildren(List<float[]> newEqList, float t, float mag, int ngen,
		...
		if (ngen == 1) {
			prod = calculateProductivity(t, mag, forecastStart, forecastEnd, a_sample, b, p_sample, c, alpha,
					minMagLimit);
		} else {
			double prodCorrection = Math.log10((maxMagLimit - Mc) / (maxMagLimit - minMagLimit));
			prod = calculateProductivity(t, mag, forecastStart, forecastEnd, a_sample + prodCorrection, b, p_sample, c,
					alpha, minMagLimit);
		}
		long numNew = assignNumberOfOffspring(prod);

		// if(D) System.out.format("Parent Mag: %.2f Time: %5.2f Generation: %d Number
		// of offspring: %d %n", mag, t, (int)ngen, (int)numNew);
		if (numNew > 0 && ngen < maxGenerations) {
			...
			if (D)
				System.out.println("Sim=" + simNumber + " t=" + t + " has reached " + maxGenerations
						+ " generations. Cutting it short.");
			// if(D) System.out.println("n = " +
			// ETAS_StatsCalc.calculateBranchingRatio(a_sample, p_sample, c_sample, alpha,
			// b, forecastEnd, Mc, maxMagLimit)
			if (D)
				System.out.println("n = "
						+ ETAS_StatsCalc.calculateBranchingRatio(a_sample, p_sample, c, alpha, b, forecastEnd,
								minMagLimit, maxMagLimit)
						+ " a=" + a_sample + " p=" + p_sample + " c=" + c + " (" + c_sample + ")" + " al=" + alpha
						+ " b=" + b + " T=" + forecastEnd + " Mmin=" + minMagLimit + " Mmax=" + maxMagLimit);
		}
	...
	private double calculateProductivity(float t, float mag, double forecastStart, double forecastEnd,
		...
		if (t < forecastStart) {
			if (Math.abs(1 - p) < 1e-6)
				unscaledProductivity = Math.pow(10, a_sample)
						* (Math.log(forecastEnd - t + c) - Math.log(forecastStart - t + c));
			else
				unscaledProductivity = Math.pow(10, a_sample) / (1 - p)
						* (Math.pow(forecastEnd - t + c, 1 - p) - Math.pow(forecastStart - t + c, 1 - p));
		} else if (t < forecastEnd) {
			if (Math.abs(1 - p) < 1e-6)
				unscaledProductivity = Math.pow(10, a_sample) * (Math.log(forecastEnd - t + c) - Math.log(t + c));
			else
				unscaledProductivity = Math.pow(10, a_sample) / (1 - p)
						* (Math.pow(forecastEnd - t + c, 1 - p) - Math.pow(c, 1 - p));
		} else {
	...
	/*
	 * Returns a sample of a,p,c using the likelihood array provided. This method
	 * inverts the cumulative likelihood function,
	 * so the sum of the likelihood array needs to be normalized to 1, which it
	 * should be if using the likelihood calculator
	 * in this package.
	 * 
	 * @author Nicholas van der Elst
	 */
	private double[][] sampleParams(int nsamples, double maxMag) {
		...
		for (h = 0; h < num_ams; h++) {
			for (i = 0; i < num_a; i++) {
				for (j = 0; j < num_p; j++) {
					for (k = 0; k < num_c; k++) {
						nbranch = ETAS_StatsCalc.calculateBranchingRatio(a_vec[i], p_vec[j], c_vec[k], alpha, b,
								forecastEnd, Mc, maxMag);
						if (nbranch < 1)
						...
						if (toc > warnTime) {
							...
							timeEstimate = toc * (num_p * num_c * num_ams * num_a)
									/ ((h) * (num_p * num_c * num_a) + (i) * (num_c * num_p) + (j) * (num_c) + k);
							System.out.format(initialMessageString + "Approximately %d seconds remaining...\n",
									(int) ((timeEstimate - toc)));
							initialMessageString = "...";
						}
	...
	private double assignMagnitude(double b, double minMag, double Mmax) {
		...
		// double mag = minMag - Math.log10(1.0 - u*(1.0 - Math.pow(10,
		// -b*(Mmax-minMag))))/b;
		// return mag;
	...
	// for(int i = 0; i < nSims; i++){
	// eqCat = getETAScatalog(i); //double[] eqCat = {relativeTime, magnitude,
	// generationNumber}
	// for(int j = 0; j < eqCat.size(); j++) {
	// allCatalogList.add(eqCat.get(j));
	// }
	// }
	// if(D) System.out.println("allCatalogList contains " + allCatalogList.size() +
	// " elements.");
	// if(D) System.out.println("last eqCat contains " + eqCat.size() + "
	// elements.");
	...
	// for(float[] eq: eqList){
	// paragraph.append(String.format("%5.2f %5.2f %d %n", eq[0], eq[1],
	// (int)eq[2]));
	// }
	...
	private double getMeanAms() {
		return ams_vec[(int) (ams_vec.length / 2)];
	}
}
