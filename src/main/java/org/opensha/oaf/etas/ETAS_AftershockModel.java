 ...
 * Subclasses should (sometimes) normalize Likelihood values so they sum to 1.0
 * over the range of parameter-values specified;
 * this can be accomplished using the convertArrayToLikelihood(double
 * maxLogLikeVal) method.
 * 
 * @author field
 * @author van der Elst
 *
...
public abstract class ETAS_AftershockModel {
	...
	protected double min_ams, max_ams, delta_ams = 0, min_a, max_a, delta_a = 0, min_p, max_p, delta_p = 0, min_c,
			max_c, delta_c = 0; // grid search not used for forecast
	...
	/**
	 * This converts the likelihood from log-likelihood to likelihood values, making
	 * sure NaNs do not occur
	 * due to high logLikelihoods, and re-normalizes the likelihood so as values sum
	 * to 1.0.
	 * 
	...
	protected double convertLogLikelihoodArrayToLikelihood(double maxLogLikeVal) {
		double total = 0;
		// double corr = 0;
		// if(maxLogLikeVal>100.0) // values above ~700 cause NaNs from Math.exp(), so
		// we subtract some number from all values to avoid such numerical problems
			...
			for (int aIndex = 0; aIndex < num_a; aIndex++) {
				for (int pIndex = 0; pIndex < num_p; pIndex++) {
					for (int cIndex = 0; cIndex < num_c; cIndex++) {
						...
						// System.out.println("NaN encountered in likelihood for index [" + amsIndex + "
						// " + aIndex + " " + pIndex + " " + cIndex + "]" );
	 ...
	 * This converts the likelihood from log-likelihood to likelihood values, making
	 * sure NaNs do not occur
	 * due to high logLikelihoods, but does NOT re-normalize the likelihood so as
	 * values sum to 1.0.
	 * 
	 * @param maxLogLikeVal - the maximum values in the input likelihood
	 * @return
	 */
	protected double convertLogLikelihoodArrayToLikelihood_nonNormalized(double[][][][] likelihood,
			double maxLogLikeVal) {
		double total = 0;
		// double corr;
		double like, loglike;
		// if(maxLogLikeVal>100.0) // values above ~700 cause NaNs from Math.exp(), so
	...
	public double getMaxLikelihood_ams() {
		return get_ams(max_ams_index);
	}

	public double getMaxLikelihood_a() {
		return get_a(max_a_index);
	}

	public double getMaxLikelihood_p() {
		return get_p(max_p_index);
	}

	public double getMaxLikelihood_c() {
		return get_c(max_c_index);
	}

	protected double get_ams(int amsIndex) {
		return ams_vec[amsIndex];
	}

	protected double get_a(int aIndex) {
		return a_vec[aIndex];
	}

	protected double get_p(int pIndex) {
		return p_vec[pIndex];
	}

	protected double get_c(int cIndex) {
		return c_vec[cIndex];
	}
	...
	public double get_bSigma() {
		return bSigma;
	}

	public ETAScatalog getSimulatedCatalog() {
		return simulatedCatalog;
	}

	public void set_bSigma(double bSigma) {
		this.bSigma = bSigma;
	}
	...
	/**
	 * This computes the distribution of the number of M≥forecastMag events for the
	 * suite of simulated ETAS catalogs
	 * 
	 * @param tMinDays
	 * @param tMaxDays
	 * @param forecastMag
	...
	/**
	 * This gives the number of aftershocks associated with the maximum likelihood
	 * a/p/c parameters (which represents the mode
	 * in the number of events space) above the given minimum magnitude and over the
	 * specified site span. A GR distribution with
	...
	public double getModalNumEvents(double magMin, double tMinDays, double tMaxDays) {
		// num_DistributionFunc = computeNum_DistributionFunc( tMinDays, tMaxDays,
		// magMin);
	...
	public void generateStochasticCatalog(double dataMinDays, double dataMaxDays, double forecastMinDays,
			double forecastMaxDays, int nSims) {
		generateStochasticCatalog(dataMinDays, dataMaxDays, forecastMinDays, forecastMaxDays, nSims, true);

	}

	public void generateStochasticCatalog(double dataMinDays, double dataMaxDays, double forecastMinDays,
			double forecastMaxDays, int nSims, boolean dynamicMinMag) {
		if (D) {
			System.out.println("Computing Forecast with " + nSims + " simulations (Data window: " + dataMinDays + " "
					+ dataMaxDays + ", Forecast window: " + forecastMinDays + " " + forecastMaxDays + ")");
			System.out.println(
					"Model Params: " + getMaxLikelihood_ams() + " " + getMaxLikelihood_a() + " " + getMaxLikelihood_p()
							+ " " + getMaxLikelihood_c() + " " + alpha + " " + b + " " + magComplete + " " + refMag);
		}
		...
		if (dynamicMinMag) {
			...
			if (nSims > 0) {
				...
				while (numNotZero < 95 & minMag > -3.0) {
					minMag--;
					simulatedCatalog = new ETAScatalog(ams_vec, a_vec, p_vec, c_vec, epiLikelihood, alpha, b, refMag,
							mainShock, aftershockList, dataMinDays, dataMaxDays, forecastMinDays, forecastMaxDays,
							magComplete, minMag, maxMag, maxGenerations, nCalibrationSims, validate); // maxMag = 9.5,
																										// maxGeneratons
																										// = 100;
					numNotZero = 0;
		...
		try {
			simulatedCatalog = new ETAScatalog(ams_vec, a_vec, p_vec, c_vec, epiLikelihood, alpha, b, refMag,
					mainShock, aftershockList, dataMinDays, dataMaxDays, forecastMinDays, forecastMaxDays, magComplete,
					minMag, maxMag, maxGenerations, nSims, validate); // maxMag = 9.5, maxGeneratons = 100;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ArbitrarilyDiscretizedFunc getModalCumNumEventsWithLogTime(double magMin, double tMinDays, double tMaxDays,
			int numPts) {
		return getFractileCumNumEventsWithLogTime(magMin, tMinDays, tMaxDays, numPts, 0.5);
	}

	// get forecast number from simulation
	public ArbitrarilyDiscretizedFunc getFractileCumNumEventsWithLogTime(double magMin, double tMinDays,
			double tMaxDays, int numPts, double fractile) {
	...
	public ArbitrarilyDiscretizedFunc getExpectedNumEventsWithLogTime(double magMin, double tMinDays, double tMaxDays,
			int numPts) {
		...
		subList = aftershockList.getRupsAboveMag(magMin)
				.getRupsBetween((long) (tMinDays * ETAS_StatsCalc.MILLISEC_PER_DAY),
						(long) (tMaxDays * ETAS_StatsCalc.MILLISEC_PER_DAY));
		...
		// use a minimum of numPts pts, but start with the times of the actual
		// aftershocks
		if (totalCount < numPts) {
			// pad it with log and lin-spaced points
			if (D)
				System.out.println("padding time vector");
			...
			if (D)
				System.out.println("thinning time vector");
	...
	public double getExpectedNumEvents(double magMin, double tMinDays, double tMaxDays) {
		...
		// double k = Math.pow(10, ams + alpha*(mainShock.getMag() - refMag) + b*(refMag
		// - magComplete));
		...
		for (int i = 0; i < Nas; i++) {
			...
			// productivity[i] = Math.pow(10, a + alpha*(magAftershocks[i] - refMag) +
	...
	// public EvenlyDiscretizedFunc[]
	// getCumNumMFD_FractileWithAleatoryVariability(double[] fractileArray, double
	// minMag, double maxMag, int numMag, double tMinDays, double tMaxDays) {
	// EvenlyDiscretizedFunc[] mfdArray = new
	// EvenlyDiscretizedFunc[fractileArray.length];
	//
	// for(int i=0;i<fractileArray.length;i++) {
	// mfdArray[i] = new EvenlyDiscretizedFunc(minMag, maxMag, numMag);
	// mfdArray[i].setName(fractileArray[i]+" Fractile for Num Events, including
	// aleatory variability");
	// mfdArray[i].setInfo("Cumulative distribution (greater than or equal to each
	// magnitude)");
	// }
	//
	// // get fractile rates at magComplete
	// // double mag0 = mfdArray[0].getX(0); // any MFD will do, as they all have
	// the same x-axis values
	//// double mag0 = magComplete;
	//// double[] valsArray0 = getCumNumFractileWithAleatory(fractileArray, mag0,
	// tMinDays, tMaxDays);
	////
	//// double mag, val;
	//// for(int i=0;i<numMag;i++) {
	//// mag = mfdArray[0].getX(i); // any MFD will do, as they all have the same
	// x-axis values
	...
	// int[] eventCounts =
	// simulatedCatalog.getEventCounts(tMinDays,tMaxDays,minMag);
	...
	// public List<EvenlyDiscretizedFunc[]> getMNDandMPOE(double[] fractileArray,
	// double minMag, double maxMag, int numMag, double tMinDays, double tMaxDays) {
	// List<EvenlyDiscretizedFunc[]> forecast = new
	...
	// magnitudePDF[0].setInfo("Cumulative distribution (greater than or equal to
	// each magnitude)");
	//
	// // use the stochastic method
	// int[] eventCounts =
	...
	// StackedMND mndStack = new StackedMND(mfdArray, eventCounts, minMagSim, b,
	// fractileArray);
	...
	// }
	//
	// public double[] getCumNumFractileWithAleatory(double[] fractileArray, double
	// mag, double tMinDays, double tMaxDays) {
	...
	// double minMagSim = simulatedCatalog.minMagLimit;
	// StackedMND mndStack = new StackedMND(mag, eventCounts, minMagSim, b,
	// fractileArray);
	...
	public List<EvenlyDiscretizedFunc[]> getMNDandMPOE(double[] fractileArray, double minMag, double maxMag, int numMag,
			double tMinDays, double tMaxDays) {
		...
		mfdArray = getCumNumMFD_FractileWithAleatoryVariability(fractileArray, minMag, maxMag, numMag, tMinDays,
				tMaxDays);
	...
	public EvenlyDiscretizedFunc[] getCumNumMFD_FractileWithAleatoryVariability(double[] fractileArray, double minMag,
			double maxMag, int numMag, double tMinDays, double tMaxDays) {
		EvenlyDiscretizedFunc[] mfdArray = new EvenlyDiscretizedFunc[fractileArray.length];
		...
		// get fractile rates at magComplete
		// double mag0 = mfdArray[0].getX(0); // any MFD will do, as they all have the
		// same x-axis values
		// double mag0 = magComplete;
		// double[] valsArray0 = getCumNumFractileWithAleatory(fractileArray, mag0,
		// tMinDays, tMaxDays);
	...
	// I thought I could make this one faster... still trying
	...
	// }
	//
	// // get the super catalog for this time range and minimum magnitude
	// List<List<float[]>> catalogList = new ArrayList<List<float[]>>();
	// List<float[]> eqCat;
	// float[] eqTemp;
	// for(int i=0;i<simulatedCatalog.nSims;i++) {
	// eqCat = simulatedCatalog.getETAScatalog(i);
	// for(int j = 0; j < eqCat.size(); j++) {
	// eqTemp = eqCat.get(j);
	// if (eqTemp[0] < tMinDays || eqTemp[0] > tMaxDays || eqTemp[1] < minMag)
	// eqCat.remove(j);
	// }
	// catalogList.add(eqCat);
	// }
	//
	// // now work on this catalog, removing elements as we go
	//
	// double mag, val;
	// int[] eqCount = new int[simulatedCatalog.nSims];
	// for(int i=0;i<numMag;i++) {
	// List<List<float[]>> catalogList2 = new ArrayList<List<float[]>>();
	//
	// mag = mfdArray[0].getX(i); // any MFD will do, as they all have the same
	// x-axis values
	//
	// for(int j = 0; j < catalogList.size(); j++) {
	// eqCat = catalogList.get(j);
	//
	// for(int k = 0; k < eqCat.size(); k++) {
	// eqTemp = eqCat.get(k);
	//
	// if (eqTemp[1] < mag)
	// eqCat.remove(k);
	// }
	//
	// eqCount[j] = eqCat.size();
	// catalogList2.add(eqCat);
	// }
	//
	// Arrays.sort(eqCount);
	//
	// for(int j=0;j<fractileArray.length;j++) {
	// int index = (int) Math.round(simulatedCatalog.nSims * fractileArray[j]);
	// val = eqCount[index];
	// mfdArray[j].set(i,val);
	// }
	//
	...
	public double[] getCumNumFractileWithAleatory(double[] fractileArray, double mag, double tMinDays,
			double tMaxDays) {
		// can I compute this just once?
		...
		for (int i = 0; i < fractileArray.length; i++) {
			...
			// the preceding gives the number of magComplete used in the simulation. We want
			// the number of mag. Scale:
			if (mag < simulatedCatalog.minMagLimit) {

				// fractValComplete is an integer (the number of earthquakes for which f% of the
				// simulations have fewer)
				// this can be zero, which is no good for scaling the rate to magnitudes lower
				// than mag (or magComplete)
				// It is therefore necessary to produce some kind of fractional count. We do
				// this by computing the
				// Poisson rate that gives the observed probability of zero events in the
				// simulations.

				// if(D) System.out.println("mag=" + mag + " duration=" + (tMaxDays-tMinDays) +
				// " fractValComplete=" + fractValComplete);
				...
				if (fractValComplete <= 1) {
					...
					// if(D) System.out.println("new fractValComplete=" + fractValComplete + "
					// probOne=" + probOne + " lambda=" + lambda);
				}
	...
	public double getMedianPoissInterp(double mag, double tMinDays, double tMaxDays) {
		...
		// find the probability of exceeding the discrete median value (less than 50%,
		// usually)
		double prob = 1;
		...
		if (D)
			System.out.println(
					"new discreteMedian=" + discreteMedian + " prob of exceeding median=" + prob + " lambda=" + lambda);
	...
	/**
	 * Computes the "Poissonian" quantile using a continuous gamma distribution to
	 * get non-integer quantiles.
	...
	// /** This one gives probability as a function of magnitude for the specified
	// time range
	// *
	// **/
	// public EvenlyDiscretizedFunc getMagnitudePDFwithAleatoryVariability(double
	// minMag, double maxMag, int numMag, double tMinDays, double tMaxDays) {
	// EvenlyDiscretizedFunc magnitudePDF = new EvenlyDiscretizedFunc(minMag,
	// maxMag, numMag);
	// magnitudePDF.setName("probabiltiy of exceeding magnitude M");
	// magnitudePDF.setInfo("Cumulative distribution (greater than or equal to each
	// magnitude)");
	//
	//// double value = getProbabilityWithAleatory(minMag, tMinDays, tMaxDays);
	//
	// for(int i=0;i<numMag;i++) {
	// double mag = magnitudePDF.getX(i); // any MFD will do, as they all have the
	// same x-axis values
	...
	// public double[] getProbabilityWithAleatory(int[] number, double mag, double
	// tMinDays, double tMaxDays) {
	// computeNum_DistributionFunc(tMinDays, tMaxDays, mag);
	//
	//
	...
	// // the above probability is the fraction of simulations with events above
	// max(magComplete, mag),
	// // so if mag<magComplete, we need to scale up the probability. We do this
	// with a Poisson rate assumption.
	//
	// if(mag < simulatedCatalog.minMagLimit){
	// prob[k] = 1 - Math.pow(1-prob[k], Math.pow(10,
	// -b*(mag-simulatedCatalog.minMagLimit)));
	// }
	//
	// }
	//
	...
	public double getProbabilityWithAleatory(double mag, double tMinDays, double tMaxDays) {
		...
		// the above probability is the fraction of simulations with events above
		// max(magComplete, mag),
		// so if mag<magComplete, we need to scale up the probability. We do this with a
		// Poisson rate assumption.
		// if(mag < magComplete){
	...
	/**
	 * This returns the PDF of ams, which is a marginal distribution if either c or
	 * p
	...
	public HistogramFunction getPDF_ams() {
		if (num_ams == 0) {
			...
			if (D)
				System.out.println("ams: " + getMaxLikelihood_ams() + " " + sigma_ams + " " + min_ams + " " + max_ams
						+ " " + num_ams);
			...
			hist.setName(name);
			// if(D) System.out.println("PDF of ams-value: totalTest =
	 ...
	 * a is constrained (num_a=1). (how important is this? I want to show pdf with
	 * one value)
	 * 
	 * @return
	 */
	public HistogramFunction getPDF_a() {
		if (num_a == 0) {
			...
			if (D)
				System.out.println(
						"a: " + getMaxLikelihood_a() + " " + sigma_a + " " + min_a + " " + max_a + " " + num_a);
			...
			if (num_a == 1)
				...
				name += " (marginal)";
			hist.setName(name);
			// if(D) System.out.println("PDF of a-value: totalTest =
			// "+hist.calcSumOfY_Vals());
	...
	public HistogramFunction getPDF_p() {
		if (num_p == 0) {
			...
			if (D)
				System.out.println(
						"p: " + getMaxLikelihood_p() + " " + sigma_p + " " + min_p + " " + max_p + " " + num_p);
			...
			if (num_p == 1)
				...
				name += " (marginal)";
			hist.setName(name);
			// if(D) System.out.println("PDF of p-value: totalTest =
			// "+hist.calcSumOfY_Vals() + " Max = " + hist.getMaxY());
	...
	public HistogramFunction getPDF_c() {
		if (num_c == 0) {
			...
			if (D)
				System.out.println(
						"c: " + getMaxLikelihood_c() + " " + sigma_logc + " " + min_c + " " + max_c + " " + num_c);
			...
			if (num_c == 1)
				...
				name += " (marginal)";
			hist.setName(name);
			// if(D) System.out.println("PDF of c-value: totalTest =
			// "+hist.calcSumOfY_Vals());
	...
	/**
	 * This returns the PDF of logc, which is a marginal distribution if either a or
	 * p
	 * are unconstrained (either num_a or num_p not equal to 1). Null is returned if
	 * c is constrained (num_c=1).
	 * 
	...
	public HistogramFunction getPDF_logc() {
		if (num_c == 0) {
			...
			if (D)
				System.out.println("c: " + getMaxLikelihood_c() + " " + sigma_logc + " " + min_logc + " " + max_logc
						+ " " + num_c);
			...
			if (num_c == 1)
				...
				name += " (marginal)";
			hist.setName(name);
			// if(D) System.out.println("PDF of logc-value: totalTest =
			// "+hist.calcSumOfY_Vals());
	 ...
	 * this one isn't ever used because c is log-normal. Use
	 * get2D_PDF_for_a_and_logc() instead.
	 * 
	 * @return
	 */
	...
	public EvenlyDiscrXYZ_DataSet get2D_PDF_for_a_and_logc() {
		if (num_a == 1 || num_c == 1) {
			...
			EvenlyDiscrXYZ_DataSet hist2D = new EvenlyDiscrXYZ_DataSet(num_a, num_c, min_a, min_logc, delta_a,
					delta_logc);
			for (int amsIndex = 0; amsIndex < num_ams; amsIndex++) {
				for (int aIndex = 0; aIndex < num_a; aIndex++) {
					for (int pIndex = 0; pIndex < num_p; pIndex++) {
	...
	public EvenlyDiscrXYZ_DataSet get2D_PDF_for_logc_and_p() {
		if (num_c == 1 || num_p == 1) {
			...
			EvenlyDiscrXYZ_DataSet hist2D = new EvenlyDiscrXYZ_DataSet(num_c, num_p, min_logc, min_p, delta_logc,
					delta_p);
			for (int amsIndex = 0; amsIndex < num_ams; amsIndex++) {
				for (int aIndex = 0; aIndex < num_a; aIndex++) {
					for (int pIndex = 0; pIndex < num_p; pIndex++) {
	...
	/*
	 * This one returns the value of the cdf at the specified x-value, with a
	 * randomized correction for the discreteness of
	 * the distribution. The fractile value for count n is defined q = P(N <= n) -
	 * rand(1)*P(N == n);
	 */
	public double getCumulativeQuantileValue(double tmin, double tmax, double minMag, int nObserved) {
		...
		// if(D) System.out.println(this.num_DistributionFunc + " " +
		// this.num_DistributionFunc.getNormalizedCumDist() + " " + nObserved + " " +
		// ySum);
